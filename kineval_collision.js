...
function robot_collision_forward_kinematics(q){
    var mt = generate_translation_matrix(q[0], q[1], q[2]);
    var mtemp = matrix_multiply(generate_rotation_matrix_Z(q[4]), generate_rotation_matrix_Y(q[5]));
    mtemp = matrix_multiply(mtemp, generate_rotation_matrix_X(q[3]));
    var nicole = matrix_multiply(mt,mtemp);
    if (robot.links_geom_imported){
        var geom = matrix_multiply(generate_rotation_matrix_Y(-Math.PI/2),generate_rotation_matrix_X(-Math.PI/2));
        nicole = matrix_multiply(nicole, geom);

    }
    return traverse_collision_forward_kinematics_link(robot.links[robot.base], nicole, q);
}
